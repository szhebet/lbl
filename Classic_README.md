...
2. В nginx каждого инстанса включить `ssl_verify_client optional` (а лучше - принудительный `ssl_verify_client`) с
...
«Запросы» жмёт **«Запросить по федерации»** и фоновый распределитель
...
На входящую заявку соседа (вкладка «Запросы соседей») администратор может предложить любую книгу из своего каталога.
...
пометкой «предложено администратором».
...
1. На сервере создать директорию `apk/` в корне проекта (или настроить свой путь через `config.toml`).
2. Положить в неё файлы:

```
apk/
├── library.apk        # signed release APK
├── version.txt        # версия, например "1.2", должна соответствовать версии в .apk.conf на момент сборки
```

3. Добавить в `config.toml` сервера:
...
### Настройка mTLS
Все уже сделано в .sh файлах сборки, описание ниже - для пояснения, как это работает.
