...
- DB version tracked in `db_version` table; current version: `5.7`
...
- **Форма списка чтения: прокрутка модалки + фикс несохранения полей после «Связать»**:
  ...
  - E2E-проверка сценария без jsdom (реестр недоступен): Node-скрипт извлекает реальные функции из `app.js` (`apiFetch`, offers-блок, `applyBookToReadlistForm`) через маркеры и гоняет их в vm-песочнице с DOM-стабом против живого сервера: register/login → create readlist → вставка `fed_offers` (local_edition_id=875) → `linkReadlistOffer()` → ассерты заполнения полей → PUT как делает submit-хендлер → GET подтверждает bookname/author/book_id в БД УЖЕ ПРИ ПЕРВОМ сохранении. Тестовые данные вычищены.
  - Проверено: `go test -count=1 ./src/` зелёный, `node --check static/js/app.js` OK, APK-ассеты синхронизированы, бинарь пересобран, образ пересоздан, `library-app` (9091) и `library-app2` (9092) перезапущены (200), nginx 443/444/445 → 200, правило CSS и хелпер отдаются сервером, контейнеры оставлены запущенными.
- **Федеративные предложения книг: first-offer-wins, список офферов у пользователя, остановка рассылки + фикс 400 на «Связать» (миграция 5.7)**:
  - Миграция `src/migration_5.7.sql`: таблица `fed_offers` — журнал всех предложений по каждому запросу (`read_list_id` UUID FK CASCADE, `source_url`, `remote_work_id`, `remote_edition_id`, `local_edition_id` FK editions, `title`, `authors`, `received_at` TIMESTAMPTZ, `linked` BOOLEAN; UNIQUE(read_list_id, source_url, remote_edition_id)). Подключена в `src/main.go` (`embeddedMigration57`, `currentDBVersion = "5.7"`).
  - First-offer-wins: `linkOfferToReadList` заменён на `acceptFedOffer` (`src/server_api.go`) — оффер журналируется всегда, а привязка к `read_list.book_id` и простановка `fulfilled_at/by_url` делаются атомарным `UPDATE ... WHERE book_id IS NULL`, т.е. выигрывает первый ответивший сервер, остальные книги просто импортируются в библиотеку без перезаписи привязки. После первой привязки `cancelPendingFedDeliveries` отменяет pending/failed outbox-строки этого запроса (`last_error='Ответ от сервера уже получен'`), а `gatherApprovedRequests` больше не собирает запросы с `fulfilled_at IS NOT NULL` — рассылка остальным соседам прекращается. Повторное «Запросить по федерацию» сбрасывает uid/fulfilled_at, позволяя начать заново.
  - Список офферов для пользователя: `GET /api/v1/user/readlist/:id/offers` (владелец; элементы с `linked`, `received_at`, сортировка linked DESC → received_at DESC) и `POST /api/v1/user/readlist/:id/offers/link` (`{offer_id}`) — выбор другой книги из уже полученных (перевыставляет linked-флаги, `fulfilled_by_url` следует за выбранным). Фронтенд: блок «Полученные книги» внизу модалки списка чтения (`templates/index.html` `#rlOffersBlock`), `static/js/app.js`: `loadReadlistOffers`/`renderReadlistOffers`/`formatOfferDateTime` (DD.MM.YYYY HH:MM)/`linkReadlistOffer` (делегирование `.rl-link-offer-btn`, CSP-совместимо), стили `.rl-offer-*` в `static/css/style.css`.
  - **Фикс 400 при «Связать»**: SPA шлёт `{"offer_id":"3"}` строкой (значение из DOM dataset), а gin-поле было `int64` → биндинг отклонял тело с «Некорректный запрос». Исправлено с двух сторон: фронт присылает `parseInt(offerId, 10)`; бэкенд получил тип `flexID` (`src/reading.go`) с `UnmarshalJSON`, принимающим и число, и числовую строку.
  - Тесты `src/fed_offers_test.go`: `TestServerOfferFirstWinsAndOffersAPI` (2 соседа предлагают разные книги — привязывается первый, второй импортируется в библиотеку, API отдаёт оба), `TestFedDeliveryCancelledAfterOffer` (после оффера pending outbox → cancelled и не пересоздаётся), `TestLinkReadListOfferBodyFormats` (строковый/числовой offer_id, чужой/несуществующий id → 404). Хелпер `cleanupOfferFixture`.
  - **Паттерн очистки тестов**: `t.Cleanup` не работает для DB-фикстур (пул закрывается раньше колбэков) — очистка делается в `defer func(){ cleanup...; db.Close() }()`, что срабатывает и при раннем выходе через `require.*FailNow`. Lookup-запросы в тестах скопированы по своему `read_list_id`, чтобы старый мусор не ломал выборку.
  - Live E2E на app2 (445): регистрация → создание списка → вставка fed_offers → `POST /offers/link` со строковым id → **HTTP 200**, `book_id` привязан, GET offers показывает `linked:true`; ранее тот же запрос давал 400. Тестовые данные вычищены.
  - Проверено: `go test -count=1 ./src/` зелёный, `node --check static/js/app.js` OK, APK-ассеты (`app.js`, `index.html`, `style.css`) синхронизированы, бинарь пересобран, образ пересоздан, `library-app` (9091) и `library-app2` (9092) перезапущены (200), nginx 443/444/445 → 200, контейнеры оставлены запущенными.
