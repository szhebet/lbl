...
- **Федерация библиотек** — объединение нескольких независимых инстансов в сеть: поиск книги по каталогам соседних серверов («Поиск 1й круг») и импорт найденной книги с сохранением исходных ID, конфликтов и взаимным TLS
...
│   ├── federation.go          # Федеративный поиск / импорт / тест связи
│   ├── server_api.go          # Серверная роль (/api/v1/server/*)
...
│   ├── migration_*.sql        # Миграции (текущая версия: 4.9)
│   ├── config/
