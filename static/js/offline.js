(function() {
    ...
    window.SyncService = {
        ...
        async sync(force) {
            ...
            var now = Date.now();
            if (!force && now - this._lastSyncAt < this._minInterval) {
                debug('sync skipped (min interval), last=' + this._lastSyncAt);
                return;
            }
            if (!isOnline()) {
                debug('offline, sync skipped');
    ...
    function init() {
        ...
        var uid = getCurrentUserId();
        if (uid === null) {
            debug('no user, deferring init');
            // Try again periodically until user is logged in
        ...
        if (hasOtherUserData) {
            ...
            // Reload fresh from bridge (which now has only other user's data)
            ReadListStore._loadFromBridge();
            // Re-clear user data again to be safe
            ReadListStore.clearByUser(uid);
        }
